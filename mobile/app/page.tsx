...
function GuestEditor({ weddingId, guest, groups, onClose, onSaved }: { weddingId: string; guest: { id: string; name: string; email: string | null; phone: string | null; guest_type: string; attendance_status: string; invitation_status: string; invitation_group_id: string | null; attends_dinner: boolean; dietary_notes: string | null; notes: string | null } | null; groups: InvitationGroup[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function remove() { if (!guest || !window.confirm(`Biztosan törlöd ezt a vendéget: ${guest.name}? Ez a művelet nem vonható vissza.`)) return; setBusy(true); setMessage(''); const result = await supabase!.from('guests').delete().eq('id', guest.id).eq('wedding_id', weddingId); setBusy(false); if (result.error) setMessage(result.error.message); else onSaved(); }
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="editor-card"><div className="editor-heading"><div><p className="eyebrow">{guest ? 'SZERKESZTÉS' : 'ÚJ MEGHÍVOTT'}</p><h2>{guest ? guest.name : 'Vendég felvétele'}</h2></div><button onClick={onClose} aria-label="Bezárás">×</button></div><form onSubmit={save}>
    ...
    <label>Étrendi igény<textarea value={form.dietary_notes} onChange={(e) => field('dietary_notes', e.target.value)} /></label><label>Megjegyzés<textarea value={form.notes} onChange={(e) => field('notes', e.target.value)} /></label>{message && <p className="form-message">{message}</p>}<div className="editor-actions"><button className="primary-action" disabled={busy}>{busy ? 'Folyamatban…' : 'Mentés'}</button>{guest && <button className="danger-action" type="button" disabled={busy} onClick={remove}>Vendég törlése</button>}</div>
  </form></section></div>;
}
